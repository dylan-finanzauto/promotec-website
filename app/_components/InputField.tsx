...
export const InputField: React.FC<Props> = ({ className, type, error, ...props }) => {
  ...
  return (
    <div className="relative flex">
      <input
        className={`h-10 flex-1 border rounded-lg text-[#1F2024] px-3 py-[10px] max-w-full shadow-sm transition-all disabled:bg-slate-50 ${error ? "border-red-400 outline-red-500" : "border-[#DEE5ED] outline-secn-blue"} ${className}`}
        {...props}
