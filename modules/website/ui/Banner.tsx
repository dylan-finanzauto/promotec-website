...
const Banner: React.FC<BannerProps> = ({ titleTop, titleBottom, img }) => {
  return (
    <header className="h-[500px] relative bg-princ-blue">
      <div className="h-56 min-w-xs w-2/5 absolute top-0 translate-y-1/2 left-0 rounded-e-[40px] bg-secn-blue"></div>
      <Wrapper className="w-full h-full relative">
        ...
        <h1 className="w-sm h-56 absolute top-0 translate-y-1/2 flex flex-col justify-center leading-none">
          <span className="text-[50px] text-princ-blue font-semibold">{titleTop}</span>
